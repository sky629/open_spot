...
class LocationService(
    ...
    fun searchLocationsByRadius(
        ...
        return if (category != null) {
            locationRepository.findByCategoryAndCoordinatesWithinRadius(
                category, latitude, longitude, radiusMeters, pageable
            )
        } else {
            locationRepository.findByCoordinatesWithinRadius(
                latitude, longitude, radiusMeters, pageable
            )
        }
    }
    ...
    fun getLocationsByCategory(category: CategoryType, pageable: Pageable): Page<Location> {
        return locationRepository.findByCategoryAndIsActiveTrue(category, pageable)
    }
    ...
    fun searchLocationsByKeyword(keyword: String, pageable: Pageable): Page<Location> {
        return if (keyword.isBlank()) {
            locationRepository.findByIsActiveTrueOrderByCreatedAtDesc(pageable)
        } else {
            locationRepository.searchByKeyword(keyword.trim(), pageable)
        }
    }
    ...
    fun getRecentLocations(pageable: Pageable): Page<Location> {
        return locationRepository.findByIsActiveTrueOrderByCreatedAtDesc(pageable)
    }
    ...
    fun getLocationsByUser(userId: UUID, pageable: Pageable): Page<Location> {
        return locationRepository.findByCreatedByAndIsActiveTrue(userId, pageable)
    }
    ...
    fun getFavoriteLocationsByUser(userId: UUID, pageable: Pageable): Page<Location> {
        return locationRepository.findFavoriteLocationsByUser(userId, pageable)
    }
    ...
    fun getNearestLocations(
        ...
        return if (category != null) {
            locationRepository.findByCategoryAndCoordinatesWithinRadius(
                category, latitude, longitude, maxRadius, pageable
            ).content
        } else {
            locationRepository.findByCoordinatesWithinRadius(
                latitude, longitude, maxRadius, pageable
            ).content
        }
    }
    ...
    fun getLocationCountByCategory(): Map<CategoryType, Long> {
        return CategoryType.values().associateWith { category ->
            locationRepository.countByCategoryAndIsActiveTrue(category)
        }
    }
    ...
    fun getLocationCountInRadius(
        ...
        return locationRepository.countByCoordinatesWithinRadius(latitude, longitude, radiusMeters)
    }
