...
import com.kangpark.openspot.auth.controller.dto.response.LoginResponse
...
class OAuth2LoginSuccessHandler(
    ...
    override fun onAuthenticationSuccess(
        ...
        try {
            ...
            // 1. 로그인 처리 (사용자 등록/업데이트 및 JWT 토큰 발급)
            val loginResult = authApplicationService.login(oauth2User)
            
            // 2. 응답 생성
            val loginResponse = LoginResponse.from(loginResult)
            val apiResponse = ApiResponse.success(loginResponse)
